...
// committed and uncommitted. All mutations are first given to the uncommitted tier, which may be discarded at any
// time. However, once Commit() is called, those mutations are moved to the committed tier, and the uncommitted tier is
// cleared.
...
// Map materializes the pending mutations in the MutableMap. This also commits all uncommitted mutations.
...
func (mut MutableMap) Has(ctx context.Context, key val.Tuple) (present bool, err error) {
	value, ok := mut.uncommitted.Get(key)
	if ok {
		present = value != nil
		return
...
// Commit moves all uncommitted mutations to the underlying map.
func (mut *MutableMap) Commit(ctx context.Context) error {
	...
	mut.uncommitted.Reset()
	return nil
}

// Discard removes all uncommitted mutations.
func (mut *MutableMap) Discard(context.Context) {
	mut.uncommitted.Reset()
}
